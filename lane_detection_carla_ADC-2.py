...
# ---------------- Lane helpers ----------------
LANE_CLASS_ID = 6  # CARLA semantic seg id for RoadLines (adjust if different)

def get_edge_image(bgr_img):
...
def render_multiview(raw_rgb, sem_rgb, edge_rgb, lanes_rgb, improved_rgb):
    """
    Show five images horizontally in one pygame window.
    All inputs are HxWx3 uint8 RGB.
    """
    ...
    def to_surface(img):
        # pygame.surfarray expects (W,H), so rotate
        return pygame.surfarray.make_surface(np.rot90(img, 1))

    surfaces = [to_surface(x) for x in (raw_rgb, sem_rgb, edge_rgb, lanes_rgb, improved_rgb)]
    x = 0
    for s in surfaces:
        _pygame_screen.blit(s, (x, 0))
        x += w
...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image):
        ...
        # 2. Semantic segmentation RGB (already color converted by CARLA)
        sem_rgb = cv2.cvtColor(array.copy(), cv2.COLOR_BGR2RGB)

        # Try to recover raw semantic IDs BEFORE convert(), if this sensor is semantic_segmentation
        ids_array = None
        if self._sensors[self._index][0] == 'sensor.camera.semantic_segmentation':
            # NOTE: channel containing IDs can vary by CARLA version.
            ids_buffer = np.frombuffer(image.raw_data, dtype=np.uint8)
            ids_array = ids_buffer.reshape((image.height, image.width, 4))[:, :, 2]  # try channel 2 first

        # 3. Edge detection (from raw image)
        ...
        if ids_array is not None:
            ...
            # Fallback: simple threshold on sem_rgb to approximate lane markings (white-ish)
            hsv = cv2.cvtColor(sem_rgb, cv2.COLOR_RGB2HSV)
            lanes_mask = cv2.inRange(hsv, (0, 0, 200), (180, 40, 255))
        ...
        # Keep original surf for HUD if needed
